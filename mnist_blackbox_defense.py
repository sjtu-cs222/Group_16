...
def generate_cipher_stream(key):
  np.random.seed(key)
  stream = np.random.randint(0, 255, size=(28,28), dtype='int32')
  np.random.seed(int(time.time()))
  return stream.reshape((28,28,1))

stream = generate_cipher_stream(KEY)

def xor(a, b):
  tmp = (a*255).astype("int32")
  return np.bitwise_xor(tmp, b).astype(np.float32) / 255

def tensor_xor(x, stream):
  m = x.shape[0]
  for i in range(m):
    x[i] = xor(x[i], stream)
  return x
...
def prep_bbox(sess, x, y, x_train, y_train, x_test, y_test,
  ...
  # # input transform
  # x_trans = tf.py_func(tensor_xor, [x, stream], tf.float32)
  # print(x)
  # print(x_trans)
  # x_trans = tf.reshape(x_trans, (-1, 28, 28, 1))
  ...
  x_test = x_test_defense
  x_train = x_train_defense
...
def train_sub(sess, x, y, bbox_preds, x_sub, y_sub, nb_classes,
  ...
  for rho in xrange(data_aug):
    ...
    if rho < data_aug - 1:
      ...
      # here do the input transformations
      x_sub_prev_trans = x_sub_prev.copy()
      for i in range(len(x_sub_prev)):
        x_sub_prev_trans[i] = xor(x_sub_prev[i], stream)

      eval_params = {'batch_size': batch_size}
      bbox_val = batch_eval(sess, [x], [bbox_preds], [x_sub_prev_trans],
                            args=eval_params)[0]
...
def mnist_blackbox(train_start=0, train_end=60000, test_start=0,
  ...
  accuracies['sub'] = acc
  print('accuracy of the substitute model on clean test examples:', acc)

  # Initialize the Fast Gradient Sign Method (FGSM) attack object.
  fgsm_par = {'eps': 0, 'ord': np.inf, 'clip_min': 0., 'clip_max': 1.}
  fgsm = FastGradientMethod(model_sub, sess=sess)

  # Craft adversarial examples using the substitute
  eval_params = {'batch_size': batch_size}
  x_adv_sub = fgsm.generate(x, **fgsm_par)
  x_adv_sub = tf.stop_gradient(x_adv_sub)
  # input transformations
  x_adv_sub_trans = tf.py_func(tensor_xor, [x_adv_sub, stream], tf.float32)
  # x_adv_sub_trans = tf.reshape(x_adv_sub_trans, (-1, 28, 28, 1))
  x_adv_sub_trans.set_shape([None, 28, 28, 1])

  preds_adv = model.get_logits(x_adv_sub_trans)

  # Evaluate the accuracy of the "black-box" model on adversarial examples
  accuracy = model_eval(sess, x, y, preds_adv,
